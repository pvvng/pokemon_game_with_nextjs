...
- ### 5-1. 프로젝트 중 신경 쓴 부분
  ...
  - ##### 프론트엔드 단 예외 처리
    - 사용자들은 개발자 마음대로 움직여주는 존재가 아닙니다. 그들은 실험적이고, 도전적입니다. 개발 중 도전적인 사용자가 시도해 볼 만한 몇 가지 사실을 알아내었고, 이를 예외 처리하였습니다.
      
    - ###### 로그인 여부 확인
       : 도감 페이지 (/pokedex) , 도감 디테일 페이지(/pokedex/[pokemon_id])를 제외한 모든 페이지는 로그인 후에만 이용 가능합니다. 현재 유저의 로그인 여부를 확인하기 위해 next-auth의 getServerSession 함수를 사용하였고, 현재 로그인 한 유저의 데이터가 DB에 존재하는 경우에만 페이지를 확인 가능하도록 코드를 구성했습니다. 만약, 로그인은 하였으나 DB에 정보가 없는 유저의 경우에는 회원가입 컴포넌트를 보여주도록 코드를 구성했습니다.

    - ###### 포켓몬 밀매
      : 포켓몬 밀매 버튼을 눌렀을 때, 서버로 HTTP 요청이 전송되어 서버에서 포켓몬 삭제와 보유 금액 변경이 이루어집니다. 이용자가 여러 밀매 버튼을 연타하는 경우에는 제대로 HTTP 전송이 되지 않아 포켓몬 삭제는 이루어지지만, 골드 추가는 되지 않았습니다. 이 예외를 처리하기 위해 밀매 버튼을 클릭하면 모든 버튼을 일시적으로 bootstrap spinner를 변경하여 사용자의 버튼 연타를 막았습니다. 추가로, 이 경우에 비동기 통신과 state 변경 시간이 달라 버튼 다시 보임(DB에서 포켓몬 데이터를 삭제 완료)에도 판매한 포켓몬이 1초 가량 화면에 보였습니다. 이 과정에서 비동기 통신의 처리와 state의 변경이 다르게 동작하는 것을 알게되었습니다

    - ###### script 페이지의 예외처리
      : 유저의 진행도와 보여주는 script 컴포넌트를 일치시키기 위해 URL에 스크립트 주소를 입력하여 들어갈 경우, '잘못된 접근입니다' 텍스트를 보여줍니다.

    - 제 예외 처리 코드를 자세하게 확인하고 싶으시다면 [프론트엔드 예외처리](https://velog.io/@pvvng/%ED%94%84%EB%A1%A0%ED%8A%B8%EC%97%94%EB%93%9C%EC%97%90%EC%84%9C-%EC%98%88%EC%99%B8%EC%B2%98%EB%A6%AC) 블로그 글을 확인해주세요
...
- ### 5-2. 프로젝트 중 어려웠던 부분 && 프로젝트의 아쉬운 부분
  ...
  - ##### 이미지 로딩 속도
    - 이미지 로딩 속도가 페이지 로딩 속도보다 느린 경우가 존재합니다. 이미지 확장자를 webp로 변경하거나, React의 lazy load 기능을 활용하면 해결 가능해 보입니다. 
